...
export default function PromoterPage() {
  ...
  // 获取推广者规则类型
  const cashbackRuleType = data.promoter?.cashbackRuleType || 'type_300';
  
  // 计算已核销人数
  const verifiedCount = data.stats?.verifiedCount || 0;
  // 计算当前位置（每3人一轮，循环计算）- 300版本使用
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-indigo-950 to-slate-900">
      <div className="w-full max-w-4xl mx-auto px-4 py-6 sm:px-6">
        ...
        <div className="mb-5 rounded-2xl bg-white/5 border border-white/10 p-4 sm:p-5">
          {cashbackRuleType === 'type_100' ? (
            // 100元版本：稳定奖励计划
            <>
              {/* 标题 */}
              <div className="flex items-center gap-2 mb-3">
                <span className="text-2xl sm:text-3xl">💰</span>
                <h2 className="text-white font-semibold text-base sm:text-lg">稳定奖励计划</h2>
              </div>

              {/* 规则说明 */}
              <div className="mb-4 bg-gradient-to-br from-blue-500/10 to-cyan-500/10 rounded-xl p-3 sm:p-4 border border-blue-400/20">
              ...
              <div className="grid grid-cols-3 gap-2 sm:gap-3 mb-4">
                <div className="rounded-xl bg-white/10 p-3 sm:p-4 text-center border border-blue-400/30">
                  <p className="text-blue-200 text-xs sm:text-sm mb-2">第1次</p>
                  <p className="text-blue-400 text-xl sm:text-2xl font-bold">100元</p>
                </div>
                <div className="rounded-xl bg-white/10 p-3 sm:p-4 text-center border border-blue-400/30">
                  <p className="text-blue-200 text-xs sm:text-sm mb-2">第2次</p>
                  <p className="text-blue-400 text-xl sm:text-2xl font-bold">100元</p>
                </div>
                <div className="rounded-xl bg-white/10 p-3 sm:p-4 text-center border border-blue-400/30">
                  <p className="text-blue-200 text-xs sm:text-sm mb-2">第3次</p>
                  <p className="text-blue-400 text-xl sm:text-2xl font-bold">100元</p>
                </div>
              </div>
              ...
              <div className="text-center space-y-1.5 sm:space-y-2">
                ...
                <p className="text-blue-300 text-sm sm:text-base">
                  已获得：<span className="font-bold text-xl sm:text-2xl">{totalReward}</span> 元
                </p>
              </div>
            </>
          ) : (
